...
var nodeList = document.getElementsByName("cb");
	...
	function brandInfo(brandname) {
		window.open("/Bears/jsp/admin/information/BrandInfo.jsp?brandname="
				+ encodeURI(brandname, "UTF-8"), "stin","left=150,top=50,width=1600,height=850");
	}
</script>
</head>
<body onload="datePro()">
	...
	<ul class="pager">
		<%
			if (currentBlock > 1) {
		%>
		<li><a href="AdminBrand.jsp?page=<%=startPage - 1%>">Previous</a></li>
		<%
			}
			if (totalPage > endPage) {
		%>
		<li><a href="AdminBrand.jsp?page=<%=endPage + 1%>">Next</a></li>
		<%
			}
		%>
	</ul>
	...
	<div style="text-align: center;">
		<nav>
		<ul class="pagination">
			<li><a href="AdminBrand.jsp?page=1" aria-label="Previous"> <span
					aria-hidden="true">&laquo;</span>
			</a></li>
			<%
				for (int i = startPage; i <= endPage; i++) {
			%>
			<li><a href="AdminBrand.jsp?page=<%=i%>"><%=i%></a></li>
			<%
				}
			%>
			<li><a href="AdminBrand.jsp?page=<%=totalPage%>"
